/**
Proxy Monitor v33 (综合平均延迟 + 全探针明细 + 严格离线判定)
...
function persistConfig(){ 
  try{fs.mkdirSync(CONFIG.dataDir,{recursive:true});fs.writeFileSync(CONFIG.configFile,JSON.stringify(publicConfig(),null,2));}catch(e){} 
}
...
function ensureIpFile(){ 
  try{ 
    ...
    let st=null; 
    try{st=fs.statSync(CONFIG.ipFile);}catch(e){}
    if(st && st.isDirectory()){ 
      try{fs.rmdirSync(CONFIG.ipFile);}catch(e){return false;} 
    }
...
function persistGraveyard(){ 
  try{ fs.writeFileSync(CONFIG.graveyardFile, JSON.stringify({list:state.graveyard.list,blocked:state.blocked}) ); }catch(e){} 
}
...
function pushGrave(label,id,lastOnline,mode,reason){ 
  state.graveyard.list.push({id,label,removedAt:Date.now(),lastOnlineAt:lastOnline,mode,reason}); 
}
...
async function discover(){
  ...
  const now=Date.now();
  let lines=[]; 
  try{lines=fs.readFileSync(CONFIG.ipFile,'utf8').split(/\r?\n/);}catch(e){}
  
  const present=new Set(); 
  const domJobs=[],urlJobs=[]; 
  const adds=[];
  ...
  await mapLimit(domJobs,20,async j=>{ 
    let ips=[]; 
    try{ips=await Promise.race([dnsPromises.resolve4(j.host),timeoutPromise(4000)]);}catch(e){}
...
async function probeLatency(u){
  ...
  for(let a=0;a<2;a++){ 
    const r=await runCurl2(latCmd,ms+2500); 
    lastCode=r.code; 
    lastOut=r.out; 
    lat=parseCurlJson(r.out);
...
async function probeCustoms(u){
  ...
  for(const p of CONFIG.customProbes) {
    try{
      ...
      results.push({url:p.url,host:p.url,expect:p.expect,code:'000',segs:null,ok:false,failReason:'配置错误'}); 
    }
  }
  return {results};
}
    ...
    const workers=Array.from({length:Math.min(CONFIG.concurrency,Math.max(queue.length,1))},async()=>{
      while(queue.length){
        ...
        try{
          ...
          const point={
            t:Date.now(), ok:false, off:lat.off, customs:[], avg:null, total:null,
            colo:lat.colo, loc:lat.loc, exitIp:lat.exitIp, failReason:lat.failReason
          };
          
          if(lat.ok){
            if(CONFIG.customProbes.length){
              const cus = await probeCustoms(u);
              point.customs = cus.results;
              
              // 🌟 严格离线判定：只要一个自定义探针不通，即为离线
              const allCustomOk = cus.results.every(r => r.ok);
              if(!allCustomOk){
                point.ok = false;
                const failed = cus.results.filter(r => !r.ok).map(r => `${r.host}(${r.failReason})`).join(', ');
                point.failReason = `部分自定义探针失败: ${failed}`;
              } else {
                point.ok = true;
              }
            } else {
          ...
          // 🌟 综合平均延迟计算 (官方 + 所有自定义)
          if(point.ok){
            let sumTcp = lat.off.tcp, sumTls = lat.off.tls, sumTotal = lat.off.total, sumSrc = lat.off.src;
            let count = 1;
            for(const c of point.customs){
              if(c.segs){
                sumTcp += c.segs.tcp;
                sumTls += c.segs.tls;
                sumTotal += c.segs.total;
                sumSrc += c.segs.src;
                count++;
              }
            }
            point.avg = {
              tcp: Math.round(sumTcp / count),
              tls: Math.round(sumTls / count),
              total: Math.round(sumTotal / count),
              src: Math.round(sumSrc / count)
            };
            point.total = point.avg.total;
          }
          
          if(!(state.abort && point.ok)){
            pushHistory(u.id,point); 
            state.progress.tested++;
            log((point.ok?'✅ ':'❌ ')+u.id+(point.ok?(' 均总='+point.total+'ms'):(' 失败: '+point.failReason)));
          }
        }catch(e){ 
...
function computeQuality(points){
  ...
  const avgOf=g=>{ 
    const v=oks.map(g).filter(x=>x!=null && isFinite(x)); 
    return v.length?Math.round(v.reduce((a,b)=>a+b,0)/v.length):null; 
  };
  
  const avgTotal = avgOf(p => p.avg ? p.avg.total : null);
  const avgTcp = avgOf(p => p.avg ? p.avg.tcp : null);
  const avgTls = avgOf(p => p.avg ? p.avg.tls : null);
  const avgSrc = avgOf(p => p.avg ? p.avg.src : null);
  ...
  return{
    ...
    rate, avgTotal, avgTcp, avgTls, avgSrc, samples:recent.length
  };
}
...
function formatNodeLine(ipPort,region,q){
  ...
  return `${ipPort}#${region} | 均总:${total} | 均TCP:${tcp} | 均TLS:${tls} | 均HTTP:${src}`; 
}
...
async function uploadGithub(){
  ...
  for(const [filename,lines] of Object.entries(bodies)){
    ...
    try{ 
      ...
      if(!putRes.ok)log(`⚠️ 上传 ${fullPath} 失败: HTTP ${putRes.status}`); 
    }catch(e){} 
  }
...
const server=http.createServer(async(req,res)=>{
  ...
  try{
    ...
    if(p==='/api/ipfile' && req.method==='GET'){
      let c='';
      try{c=fs.readFileSync(CONFIG.ipFile,'utf8');}catch(e){}
