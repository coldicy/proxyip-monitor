...
const CONFIG = {
  ...
  intervalSec: parseInt(process.env.INTERVAL_SEC || '600', 10),
  ...
  autoCleanDays: parseFloat(process.env.AUTO_CLEAN_DAYS || '3'),
...
// ==================== /api/state 响应缓存（性能优化） ====================
// 原理：数据没变时直接返回缓存字符串，避免重复 buildState + JSON.stringify
// 检测中最少 2 秒重建一次；空闲时最少 4 秒重建一次；数据不变则永不重建
let stateCacheJson = null;   // 缓存的 JSON 字符串
let stateCacheTime = 0;      // 上次构建时间戳
let stateCacheDirty = true;  // 数据是否发生了变化

function invalidateStateCache() { stateCacheDirty = true; }

function getStateJson() {
  const now = Date.now();
  // 数据没变 → 直接返回缓存，不重建
  if (stateCacheJson && !stateCacheDirty) return stateCacheJson;
  // 数据变了，但距上次重建太近 → 返回旧缓存，避免高频重建
  const minInterval = state.checking ? 2000 : 4000;
  if (stateCacheJson && (now - stateCacheTime) < minInterval) return stateCacheJson;
  // 重建缓存
  stateCacheJson = JSON.stringify(buildState());
  stateCacheTime = now;
  stateCacheDirty = false;
  return stateCacheJson;
}
// ==================== /api/state 响应缓存 结束 ====================

function log(m) { state.logs.push({ t: Date.now(), m: String(m) }); if (state.logs.length > 400) state.logs = state.logs.slice(-400); }
function markDirty() { dataDirty = true; invalidateStateCache(); }
// ==================== 进程级止血: 只记日志, 不让进程崩溃重启 ====================
...
function restartGithubTimer() {
  ...
  if (mins > 0 && CONFIG.github.token && CONFIG.github.repo) {
    githubTimer = setInterval(() => {
      log('⏰ 定时触发 GitHub 上传');
      uploadGithub().catch(e => { state.github.lastError = e.message; invalidateStateCache(); log('⚠️ 定时上传失败: ' + e.message); });
    }, mins * 60 * 1000);
  }
}
// ==================== 工具 ====================
...
async function runCycle() {
  if (state.checking) return; state.checking = true; state.abort = false; invalidateStateCache();
  try {
    ...
    state.lastCycle = Date.now(); invalidateStateCache();
    ...
    saveData();
    if (CONFIG.github.auto) autoUpload().catch(e => { state.github.lastError = e.message; invalidateStateCache(); log('⚠️ 自动上传失败: ' + e.message); });
  } finally { state.checking = false; state.abort = false; invalidateStateCache(); }
}
// ==================== 优质判定（含速度下限） ====================
...
async function uploadGithub() {
  ...
  state.github.lastUpload = Date.now(); invalidateStateCache();
  if (failed > 0) {
    state.github.lastError = `${failed} 个文件上传失败`; invalidateStateCache();
    log('⚠️ ' + state.github.lastError + '（下轮自动重试）');
  } else {
    state.github.lastError = null; invalidateStateCache();
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (p === '/api/state') { res.writeHead(200, { 'Content-Type': 'application/json; charset=utf-8' }); return res.end(getStateJson()); }
    ...
    if (p === '/api/graveyard/unblock' && req.method === 'POST') {
      ...
      // 从 graveyard 列表中移除
      state.graveyard.list = state.graveyard.list.filter(g => !ids.includes(g.id));
      persistGraveyard();
      markDirty();
    ...
    if (p === '/api/upload' && req.method === 'POST') { try { return json({ ok: true, ...(await uploadGithub()) }); } catch (e) { state.github.lastError = e.message; invalidateStateCache(); log('⚠️ 手动上传失败: ' + e.message); return json({ ok: false, error: e.message }, 500); } }
