...
function parseLine(raw) {
    ...
    if (raw.startsWith('[')) {
        const m = raw.match(/^\[([^]]+)\](?::(\d+))?$/); if (!m) return null; host = m[1]; if (m[2]) port = +m[2];
    } else if (raw.includes(':') && raw.split(':').length === 2 && /^\d+$/.test(raw.split(':')[1])) {
        const p = raw.split(':'); host = p[0]; port = +p[1];
...
function curlFailText(code) {
    if (code === 28) return '超时'; if (code === 7) return '连接被拒'; if (code === 35 || code === 60 || code === 61) return 'TLS错误';
...
function computeQuality(points) {
    ...
    const mk = pre => {
        const total = avgOf(p => p[pre] && p[pre].total); if (total == null) return null;
...
async function uploadGithub() {
    ...
    for (const [filename, lines] of Object.entries(bodies)) {
        ...
        try {
            ...
            if (getRes.ok) sha = (await getRes.json()).sha; else if (getRes.status !== 404) { log(`⚠️ 查询 ${fullPath} 失败: HTTP ${getRes.status}`); continue; }
        } catch (e) { continue; }
