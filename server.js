                ...
                try {
                    ...
                    if (lat.ok) {
                        if (CONFIG.customProbes.length) {
                            ...
                            if (cus.agg) { 
                                point.ok = true; 
                                point.total = cus.agg.total; 
                            } else { 
                                point.ok = false; 
                                point.failReason = '自定义源站均未达标: ' + cus.results.map(r => `${r.host}(${r.failReason})`).join(', '); 
                            }
                        } else { 
                            point.ok = true; 
                            point.total = lat.off.total; 
                        }
                    }
...
function computeQuality(points) {
    ...
    const mk = pre => { 
        const total = avgOf(p => p[pre] && p[pre].total); 
    ...
    try { 
        ...
        const items = state.units.map(u => { 
            ...
            return { 
                ...
                // 🌟 修改：传递 customResults 供前端悬浮使用
                recent: hist.slice(-40).map(p => ({ 
                    t: p.t, ok: !!p.ok, total: p.total, off: p.off, cus: p.cus, 
                    customResults: p.customResults || [] 
                })) 
            }; 
        });
...
const server = http.createServer(async (req, res) => {
    ...
    try {
        ...
        if (p === '/api/abort' && req.method === 'POST') { 
            if (state.checking) { state.abort = true; log('⏹ 收到中断请求'); } 
            return json({ ok: true }); 
        }
