/**
Proxy Monitor v32.1 (所有探针平均值统计)
官方/自定义探针延迟合并计算平均值，统一展示与优质判定
继承: v32 延迟自洽 + 双探针分组 + 延迟柱状图
*/
    ...
    const workers=Array.from({length:Math.min(CONFIG.concurrency,Math.max(queue.length,1))},async()=>{
      while(queue.length){
        ...
        try{
          ...
          const point={t:Date.now(),ok:false,off:lat.off,cus:null,all:null,total:null,colo:lat.colo,loc:lat.loc,exitIp:lat.exitIp,failReason:lat.failReason,customResults:[]};
          
          if(lat.ok){
            const allSegs = [];
            if(lat.off) allSegs.push(lat.off);
            
            if(CONFIG.customProbes.length){
              const cus=await probeCustoms(u);
              point.customResults=cus.results; point.cus=cus.agg;
              // 🌟 收集所有成功的自定义探针延迟
              cus.results.filter(r => r.ok && r.segs).forEach(r => allSegs.push(r.segs));
            }
            
            // 🌟 计算所有探针(官方+自定义)的平均值
            if(allSegs.length > 0){
              const avg = f => Math.round(allSegs.reduce((s, r) => s + f(r), 0) / allSegs.length);
              const tcp = avg(s => s.tcp);
              const tls = avg(s => s.tls);
              const total = avg(s => s.total);
              point.all = { tcp, tls, total, src: Math.max(0, total - tcp - tls) };
              point.ok = true;
              point.total = point.all.total;
            } else {
              point.ok = false;
              point.failReason = '所有探针均未返回有效延迟';
            }
          }
...
function computeQuality(points){
  ...
  if(!recent.length)return{quality:false,rate:0,avgTotal:null,avgAll:null,avgCus:null,avgOff:null,samples:0};
  ...
  // 🌟 优先使用所有探针的平均值(all)，兜底使用自定义(cus)或官方(off)
  const avgAll=mk('all');
  const avgCus=mk('cus'); const avgOff=mk('off');
  const avgTotal=avgAll?avgAll.total:(avgCus?avgCus.total:(avgOff?avgOff.total:null));
  
  const enough=recent.length>=CONFIG.qualityWindow;
  const latOk=CONFIG.maxTotalMs<=0||(avgTotal!=null && avgTotal<=CONFIG.maxTotalMs);
  return{quality:enough && rate>=CONFIG.qualityRate && latOk,rate,avgTotal,avgAll,avgCus,avgOff,samples:recent.length};
}
...
function formatNodeLine(ipPort,region,q){
  ...
  // 🌟 TLS 也优先取所有探针的平均值
  const tls=q.avgAll && q.avgAll.tls!=null?q.avgAll.tls+'ms':(q.avgCus && q.avgCus.tls!=null?q.avgCus.tls+'ms':'?ms');
  ...
  try{ state.units=Object.values(state.nodes);
    const items=state.units.map(u=>{ const hist=state.history[u.id]||[]; const latest=hist.length?hist[hist.length-1]:null;
      return{ id:u.id,label:u.id,ip:u.ip,port:u.port,ipKind:u.kind||classifyIp(u.ip),
        ...
        // 🌟 透传 all 字段给前端柱状图使用
        recent:hist.slice(-40).map(p=>({t:p.t,ok:!!p.ok,total:p.total,all:p.all,off:p.off,cus:p.cus})) }; });
