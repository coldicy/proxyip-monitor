    ...
    const workers=Array.from({length:Math.min(CONFIG.concurrency,Math.max(queue.length,1))},async()=>{
      while(queue.length){
        ...
        try{
          ...
          const point={t:Date.now(),ok:false,off:lat.off,cus:null,total:null,avgTcp:null,avgTls:null,avgHttp:null,
          ...
          if(lat.ok){
            ...
            if(customOk){
              ...
              if(avg){
                point.total=avg.total;
                point.avgTcp=avg.tcp;
                point.avgTls=avg.tls;
