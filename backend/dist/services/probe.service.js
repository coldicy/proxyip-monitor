...
class ProbeService {
    ...
    async probeLatency(node) {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
            const response = await (0, helpers_1.runCurlWithCode)(curlCmd, timeoutMs + 2500);
            lastCode = response.code;
            lastOutput = response.out;
            latencyData = (0, helpers_1.parseCurlJson)(response.out);
        ...
        if (latencyData && latencyData.http === 200) {
            ...
            const info = (0, helpers_1.parseTrace)(traceOutput);
            ...
            result.failReason = `不具备反代 CF 能力 (${(0, helpers_1.getCurlFailText)(lastCode)})`;
        }
        return result;
    }
    async probeCustomProbes(node) {
        const results = [];
        const timeoutMs = this.config.timeoutSec * 1000;
        for (const probe of this.config.customProbes) {
            try {
                ...
                const response = await (0, helpers_1.runCurlWithCode)(cmd, timeoutMs + 2500);
                const data = (0, helpers_1.parseCurlJson)(response.out);
                const code = data ? String(data.http) : '000';
                const segs = this.buildTimingSegments(data);
                let ok = false;
                let failReason = null;
                if (code === '000' && response.code !== 0) {
                    failReason = `连接失败 (${(0, helpers_1.getCurlFailText)(response.code)})`;
                }
    ...
    async probeSpeed(node) {
        ...
        const response = await (0, helpers_1.runCurlWithCode)(cmd, this.config.speedTimeoutSec * 1000 + 2500);
        const data = (0, helpers_1.parseCurlJson)(response.out);
        ...
        if (data) {
            ...
            result.failReason = `测速失败 (${(0, helpers_1.getCurlFailText)(response.code)})`;
        }
        return result;
    }
    needSpeedTest(speed) {
        if (!this.config.speedEnabled)
            return false;
        if (!speed)
            return true;
        if (speed.ok)
